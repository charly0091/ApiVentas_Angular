...
export class VentaComponent {
    ...
    this._productoServicio.lista().subscribe({
      next: data => {
        if (data.status) {
          ...
          this.listaProductos = lista.filter(producto => producto.esActivo === 1 && producto.stock > 0);
        }
      },
      error: err => {
        console.log(err);
  ...
  eliminarProducto(detalle: DetalleVenta) {
    ...
    this.listaProductosParaVenta = this.listaProductosParaVenta.filter(producto => producto.idProducto !== detalle.idProducto);
