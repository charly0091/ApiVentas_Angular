...
export class HistorialVentaComponent implements OnInit, AfterViewInit {
  ...
  opcionesBusqueda: any[] = [
    ...
    { value: 'numero', descripcion: 'NumeroVenta' },
  ];
  ...
  constructor(
    ...
    this.formularioBusqueda.get('buscarPor')?.valueChanges.subscribe((valor) => {
  ...
  aplicarFiltroTabla(evento: Event) {
    const textoFiltro = (evento.target as HTMLInputElement).value;
    this.datosListaVenta.filter = textoFiltro.trim().toLowerCase();
  }
  ...
  buscarVentas() {
    ...
    this._ventaServicio.historial(
      ...
      _fechaInicio, _fechaFin).subscribe({
        next: (data) => {
          if(data.status){
        ...
        error: (err) => {
          this._utilidadServicio.mostrarAlerta(err.error.mensaje, 'Oops');
        }
      });
  }
